      ...
      >
        ...
        {currentImage ? (
          ...
          <div className="flex flex-col items-center justify-center text-muted-foreground space-y-2">
            <UploadCloudIcon className="w-12 h-12 text-[#0EA5E9]" />
      ...
      <CardHeader className="pb-4">
        <div className="flex justify-between items-center">
          ...
          <div className="flex items-center gap-1.5">
            <Button variant="ghost" size="icon" className="text-muted-foreground hover:text-primary h-8 w-8" title="Search History (coming soon)" disabled><HistoryIcon className="w-4 h-4"/></Button>
            <Button variant="ghost" size="icon" className="text-muted-foreground hover:text-primary h-8 w-8" title="Favorites (coming soon)" disabled><HeartIcon className="w-4 h-4"/></Button>
          </div>
        </div>
      ...
      <CardContent className="space-y-4">
        ...
        {showCamera && (
          <Card className="glass-card p-3 space-y-2 mb-3" style={{borderColor: COLOR_TEAL}}>
             <div className="flex justify-between items-center">
                ...
                <Button variant="ghost" size="icon" onClick={() => setShowCamera(false)} className="h-7 w-7"><XIcon className="w-4 h-4"/></Button>
             </div>
             ...
             <Button onClick={handleCaptureImage} disabled={!hasCameraPermission || !cameraStream} className="w-full text-sm" style={{backgroundColor: COLOR_TEAL, color: 'white'}}>
                <CameraIcon className="w-4 h-4 mr-1.5"/> Capture Image
             </Button>
          </Card>
        )}
        ...
        <div className="flex items-center justify-center gap-2 mt-1">
          ...
          <Button variant="outline" size="sm" onClick={() => setShowCamera(true)} className="text-xs flex-1" style={{borderColor: COLOR_TEAL, color: COLOR_TEAL, backgroundColor: `${COLOR_TEAL}10`}}>
            <CameraIcon className="w-3 h-3 mr-1.5" /> Use Camera
             {analysisMode === 'compare' && activeUploadSlot === 1 && " (Slot 1)"}
            {analysisMode === 'compare' && activeUploadSlot === 2 && " (Slot 2)"}
          </Button>
        </div>
        ...
        <Tabs value={analysisMode} onValueChange={(value) => { setAnalysisMode(value as AnalysisMode); setAnalysisResult(null); setActiveUploadSlot(1); }} className="w-full">
          <TabsList className="grid w-full grid-cols-3 h-11 p-1" style={{backgroundColor: `hsl(var(--muted)/0.5)`}}>
            <TabsTrigger value="findSimilar" className="text-xs h-full data-[state=active]:bg-[#3B82F6] data-[state=active]:text-white data-[state=active]:shadow-md"><SearchCheckIcon className="w-3.5 h-3.5 mr-1 sm:mr-1.5"/>Find Similar</TabsTrigger>
            <TabsTrigger value="compare" className="text-xs h-full data-[state=active]:bg-[#3B82F6] data-[state=active]:text-white data-[state=active]:shadow-md"><GitCompareArrowsIcon className="w-3.5 h-3.5 mr-1 sm:mr-1.5"/>Compare</TabsTrigger>
            <TabsTrigger value="ocr" className="text-xs h-full data-[state=active]:bg-[#3B82F6] data-[state=active]:text-white data-[state=active]:shadow-md"><ScanTextIcon className="w-3.5 h-3.5 mr-1 sm:mr-1.5"/>Extract Text</TabsTrigger>
          </TabsList>
        </Tabs>
        ...
        {isLoading && !analysisResult && (
          <div className="text-center py-6 text-muted-foreground space-y-2">
            <Loader2Icon className="w-10 h-10 animate-spin mx-auto" style={{color: COLOR_BLUE}} />
